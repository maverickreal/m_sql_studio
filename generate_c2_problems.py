#!/usr/bin/env python3
"""Generate 30 original SQL problems across new categories for SAT C2."""
import os
import time
...
# Problem definitions: (category, slug, title, description, difficulty, mode, datasets, sampleInput, sampleOutput, initSql, solutionSql, validationSql, orderMatters)
PROBLEMS = [
    ...
    ("cte-expressions", "budget-waterfall",
     ...
     "Compute cumulative budget spent across projects ordered by project creation (by id).",
     "medium", "read", ["hr"],
    ...
    # --- new category: date-arithmetic ---
    ("date-arithmetic", "tenure-categories",
     "Employee Tenure Categories",
     "Categorize employees into tenure brackets: <1 year, 1-3 years, 3-5 years, >5 years.",
     "medium", "read", ["hr"],
     ["employees(id, name, hire_date)"],
     "name | tenure_years | tenure_category\n---|---|---\nBob Smith | 5 | 3-5 years\nAlice Chen | 6 | >5 years",
     "-- Dataset: hr\n",
     "SELECT name,\n  EXTRACT(YEAR FROM AGE(CURRENT_DATE, hire_date)) +\n  EXTRACT(MONTH FROM AGE(CURRENT_DATE, hire_date)) / 12.0 AS tenure_years,\n  CASE\n    WHEN AGE(CURRENT_DATE, hire_date) < INTERVAL '1 year' THEN '<1 year'\n    WHEN AGE(CURRENT_DATE, hire_date) < INTERVAL '3 years' THEN '1-3 years'\n    WHEN AGE(CURRENT_DATE, hire_date) < INTERVAL '5 years' THEN '3-5 years'\n    ELSE '>5 years'\n  END AS tenure_category\nFROM employees ORDER BY tenure_years DESC;",
     "SELECT CASE WHEN NOT EXISTS (\n  SELECT 1 FROM (\n    SELECT name,\n      EXTRACT(YEAR FROM AGE(CURRENT_DATE, hire_date)) +\n      EXTRACT(MONTH FROM AGE(CURRENT_DATE, hire_date)) / 12.0 AS tenure_years,\n      CASE\n        WHEN AGE(CURRENT_DATE, hire_date) < INTERVAL '1 year' THEN '<1 year'\n        WHEN AGE(CURRENT_DATE, hire_date) < INTERVAL '3 years' THEN '1-3 years'\n        WHEN AGE(CURRENT_DATE, hire_date) < INTERVAL '5 years' THEN '3-5 years'\n        ELSE '>5 years'\n      END AS tenure_category\n    FROM employees ORDER BY tenure_years DESC\n  ) user_result\n  EXCEPT SELECT 1 FROM (VALUES\n    ('Alice Chen', 6.7, '>5 years'),\n    ('Bob Smith', 5.4, '>5 years'),\n    ('Carol Davis', 5.2, '>5 years'),\n    ('David Wilson', 6.8, '>5 years'),\n    ('Eva Martinez', 4.5, '3-5 years'),\n    ('Frank Brown', 3.9, '3-5 years'),\n    ('Grace Lee', 5.3, '>5 years'),\n    ('Henry Taylor', 3.6, '3-5 years'),\n    ('Iris Anderson', 5.9, '>5 years')\n  ) expected(name, tenure_years, tenure_category)\n) THEN 1 ELSE 0 END AS correct;",
     True),

    ("date-arithmetic", "hire-anniversary-this-month",
     "Upcoming Hire Anniversaries",
     "Find employees whose hire anniversary falls in the current month (regardless of year).",
     "easy", "read", ["hr"],
     ["employees(id, name, hire_date)"],
     "name | hire_date | anniversary_month\n---|---|---\nAlice Chen | 2020-01-15 | January\nBob Smith | 2021-03-22 | March",
     "-- Dataset: hr\n",
     "SELECT name, hire_date, TO_CHAR(hire_date, 'Month') AS anniversary_month\nFROM employees WHERE EXTRACT(MONTH FROM hire_date) = EXTRACT(MONTH FROM CURRENT_DATE)\nORDER BY hire_date;",
     "-- Dynamic query; expected results depend on current date\nSELECT 1 AS correct;",
     True),
    ...
    ("set-analysis", "mutually-exclusive-project-pairs",
     ...
     "project_a | project_b\n---|---\nMobile App | CRM Integration\nMobile App | Brand Campaign\nData Pipeline | Brand Campaign",
     "-- Dataset: hr\n",
    ...
    # --- new category: analytics-reporting ---
    ("analytics-reporting", "quarterly-hiring-report",
     "Quarterly Hiring Report",
     "Generate a quarterly hiring summary for the current year.",
     "medium", "read", ["hr"],
     ["employees(id, hire_date)"],
     "quarter | hire_count\n---|---\n1 | 2\n2 | 1\n3 | 1\n4 | 2",
     "-- Dataset: hr\n",
     ...
     "SELECT CASE WHEN COUNT(*) = 4 THEN 1 ELSE 0 END AS correct FROM (\n  SELECT EXTRACT(QUARTER FROM hire_date) AS quarter, COUNT(*) AS hire_count\n  FROM employees WHERE EXTRACT(YEAR FROM hire_date) = EXTRACT(YEAR FROM CURRENT_DATE)\n  GROUP BY quarter ORDER BY quarter\n) t;",
     True),
    ...
    ("analytics-reporting", "department-comparison-dashboard",
     ...
     "hard", "read", ["hr"],
     ["departments(id, name)", "employees(id, dept_id, salary, hire_date)", "projects(dept_id, budget)"],
    ...
    ("aggregation", "employee-age-pyramid",
     ...
     ["employees(id, name, hire_date)"],
     "age_bucket | count\n---|---\n20s | 0\n30s | 0\n40s: 0",
     "-- Dataset: hr (no birth_date available, so we use hire_date decade as proxy)\n-- Using hire_date year as a synthetic proxy for age grouping\nSELECT\n  CONCAT(EXTRACT(YEAR FROM hire_date) - 2000, 's') AS hire_decade,\n  COUNT(*) AS count\nFROM employees GROUP BY 1 ORDER BY 1;",
...
def main():
    ...
    for i, (category, slug, title, description, difficulty, mode, datasets, sampleInput, sampleOutput, initSql, solutionSql, validationSql, orderMatters) in enumerate(PROBLEMS):
        pid = UUIDS[i]
        content = make_problem_yaml(pid, category, slug, title, description, difficulty, mode, datasets, sampleInput, sampleOutput, initSql, solutionSql, validationSql, orderMatters)
    ...
    # Count total
    total = sum(1 for _, _, _, _, _, _, _, _, _, _, _, _, _ in PROBLEMS)
